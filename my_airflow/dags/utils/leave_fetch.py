...
from utils.zoho_config import (
    ...
    CLIENT_ID_ZOHO, CLIENT_SECRET_ZOHO, REFRESH_TOKEN_LEAVE, SOURCE_ID_LEAVE
)
...
def get_source_details(access_token):
    ...
    payload = {"sourceId": SOURCE_ID_LEAVE, "includeConfiguration": True}
...
def update_source_configuration(access_token, source_details, new_config):
    ...
    clean_config.update({
        ...
        "client_refresh_token": REFRESH_TOKEN_LEAVE,
        "data_center": "in",
    ...
    print(f"🧾 Updating Zoho Source {SOURCE_ID_LEAVE} (workspace {workspace_id})...")
    resp = requests.post(url, json=payload, headers=headers)
...
def run_zoho_leave():
    try:
        ...
        access_token = AIRBYTE_PAT #get_access_token() #get_airbyte_token()
        ...
        print("⚠️ Error in run_zoho_leave:", e)
        return False
